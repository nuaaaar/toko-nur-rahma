...
class PurchaseOrderRepositoryImplement extends Eloquent implements PurchaseOrderRepository{
    ...
    */
    ...
    {
        $this->model = $this->model->select('purchase_orders.*', 'customers.name as customer_name', 'customers.phone_number as customer_phone_number', 'customers.address as customer_address')
    ...
    public function search(string $search = null)
    {
        if ($search) {
            $this->model = $this->model->where(function ($query) use ($search) {
                    ...
                    ->orWhereHas('purchaseOrderItems.product', function ($query) use ($search) {
                        $query->where('name', 'like', "%{$search}%");
                    });
            });
        }
