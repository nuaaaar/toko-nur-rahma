...
@section('title', 'Tambah Retur Customer')
...
@section('content')
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    ...
                    <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">Tambah Retur Customer</span>
                </div>
            </li>
        </ol>
    </nav>
    ...
    <div class="card">
        <div class="card-body">
            <form class="form" action="{{ route('dashboard.customer-return.store') }}" method="POST">
                @csrf
                    ...
                    <div class="col-span-6">
                        ...
                        <select name="category" id="category" class="form-control @error('category') is-invalid @enderror" required>
                            @foreach ($categories as $category)
                                <option value="{{ $category }}" {{ old('category') == $category ? 'selected' : '' }}>{{ strtoupper($category) }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-span-6">
...
@push('script')
    <script>
        ...
        let otherCustomerReturnItems = @json($sale->customerReturnsItems ?? []);
        ...
        $(document).ready(function()
        {
            $(document).on('keydown', '.scanner', function(e)
            {
                if (e.keyCode == 13)
                {
                    ...
                    {
                        showErrorDialog('Produk tidak ditemukan pada invoice penjualan');
                    }else{
                        $(this).val('');
                        $(this).focus();
                    }
            ...
            if (existingRow.length > 0) {
                ...
                let htmlRow = `
                    <tr class="sale-items" data-product-code="${product.product_code}">
                        ...
                        <td>
                            <input type="text" name="customer_return_items[index][qty]" class="form-control currency" max="${saleItem.qty - totalReturnedQty}" value="1" required>
                        </td>
                        <td>
    ...
    @foreach (old('customer_return_items') ?? [] as $customerReturnItem)
        <script>
