...
@section('content')
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    ...
                    <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">Tambah Surat Jalan</span>
                </div>
            </li>
        </ol>
    </nav>
    ...
    <div class="card">
        <div class="card-body">
            <form class="form" action="{{ route('dashboard.delivery-order.store') }}" method="POST">
                @csrf
                ...
                <div class="flex flex-col md:grid md:grid-cols-12 gap-4 mt-0">
                    ...
                    <div class="col-span-4">
                        ...
                        <input type="text" name="receiver_phone_number" id="receiver_phone_number" class="@error('receiver_phone_number') is-invalid @enderror form-control" placeholder="" value="{{ old('receiver_phone_number') ?? $purchase_order->customer->phone_number ?? '' }}"  required>
                    ...
                    <div class="col-span-4">
                        ...
                        <input type="text" name="receiver_name" id="receiver_name" class="@error('receiver_name') is-invalid @enderror form-control" placeholder="" value="{{ old('receiver_name') ?? $purchase_order->customer->name ?? '' }}"  required>
                        @error('receiver_name')
                            <span class="invalid-feedback" category="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                    ...
                    <div class="col-span-4">
                        ...
                        <input type="text" name="receiver_address" id="receiver_address" class="@error('receiver_address') is-invalid @enderror form-control" placeholder="" value="{{ old('receiver_address') ?? $purchase_order->customer->address ?? '' }}"  required>
...
@push('script')
    <script>
        ...
        let otherDeliveryOrderItems = @json($purchase_order->deliveryOrderItems ?? []);
            ...
            if (existingRow.length > 0) {
                ...
                let htmlRow = `
                    <tr class="purchase-order-items" data-product-code="${product.product_code}">
                        ...
                        <td>
                            <input type="text" name="delivery_order_items[index][qty]" class="form-control currency" max="${purchaseOrderItem.qty - totalReturnedQty}" value="1" required>
                        </td>
                        <td style="width: 1%">
    ...
    @foreach (old('delivery_order_items') ?? [] as $deliveryOrderItem)
        <script>
