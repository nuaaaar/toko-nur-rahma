...
@section('content')
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    ...
                    <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">Edit Stock Opname</span>
                </div>
            </li>
        </ol>
    </nav>
    ...
    <div class="card">
        <div class="card-body">
            ...
            @endif
            <form class="form" action="{{ route('dashboard.stock-opname.update', $stock_opname->id) }}" method="POST">
                @csrf
                @method("PUT")
                <input type="hidden" name="user_id" value="{{ request()->user()->id }}">
                <input type="hidden" name="date_from" value="{{ $stock_opname->date_from }}">
                <input type="hidden" name="date_to" value="{{ $stock_opname->date_to }}">
                <div class="flex flex-col md:grid md:grid-cols-12 gap-4 mt-0">
                    <div class="col-span-full">
                        <h5>SO Periode: {{ $stock_opname->date_from }} s/d {{ $stock_opname->date_to }}</h5>
                    </div>
                    <div class="col-span-6">
                        <label for="title" class="label-block">Judul</label>
                        <input type="text" name="title" id="title" class="@error('title') is-invalid @enderror form-control" value="{{ old('title') ?? $stock_opname->title }}" required>
                        @error('title')
                    ...
                    <div class="col-span-6">
                        ...
                        <input type="date" name="date" id="date" class="@error('date') is-invalid @enderror form-control" placeholder="" value="{{ old('date') ?? $stock_opname->date }}" min="{{ $stock_opname->date_to }}" required>
                        @error('date')
                            <span class="invalid-feedback" category="alert">
                                <strong>{{ $message }}</strong>
                            </span>
...
@push('script')
    <script>
        ...
        let stockOpnameItems = @json($stock_opname->stockOpnameItems);
        ...
        function addProductToTable(code)
        {
            ...
            let stockOpnameItem = stockOpnameItems.find(stockOpnameItem => stockOpnameItem.product_id == product.id);
            ...
            if (existingRow.length > 0) {
                ...
                let htmlRow = `
                    <tr class="stock-opname-items" data-product-code="${product.product_code}">
                        ...
                        <td>
                            <input type="text" class="form-control currency text-right trigger-qty-actual" name="stock_opname_items[index][physical]" style="min-width: 100px" value="${stockOpnameItem?.physical ?? ''}" required>
                        </td>
                        <td>
                            <input type="text" class="form-control currency text-right trigger-qty-actual" name="stock_opname_items[index][returned_to_supplier]" style="min-width: 100px"  value="${stockOpnameItem?.returned_to_supplier ?? ''}" required>
                        </td>
                        <td class="text-right actual-text">${(stockOpnameItem?.physical ?? 0) + (stockOpnameItem?.returned_to_supplier ?? 0)}</td>
                        <td class="text-right">
                            ${stockOpnameItem?.system ?? lastStock.stock}
                            <input type="hidden" name="stock_opname_items[index][system]" value="${stockOpnameItem?.system ?? lastStock.stock}">
                        </td>
                        <td class="text-right difference-text">
                            ${stockOpnameItem?.difference ?? (0 - lastStock.stock)}
                        </td>
                        <td>
                            <input type="text" class="form-control" name="stock_opname_items[index][description]" value="${stockOpnameItem?.description ?? ''}">
                        </td>
                        <td style="width: 1%">
    ...
    @foreach (old('stock_opname_items') ?? $stock_opname->stockOpnameItems as $stockOpnameItem)
        <script>
