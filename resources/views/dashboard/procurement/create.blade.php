...
@section('title', 'Tambah Pengadaan Stok')
...
@section('content')
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li aria-current="page">
                <div class="flex items-center">
                    ...
                    <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">Tambah Pengadaan Stok</span>
                </div>
            </li>
        </ol>
    </nav>
    ...
    <div class="card">
        <div class="card-body">
            <form class="form" action="{{ route('dashboard.procurement.store') }}" method="POST">
                @csrf
                ...
                <div class="flex flex-col md:grid md:grid-cols-6 gap-4 mt-0">
                            ...
                            <div class="w-2/3 grow">
                                <select name="supplier_id" id="supplier_id" class="form-control init-select2" placeholder="Pilih Agen" required>
                                    ...
                                    @foreach ($suppliers as $supplier)
                                        <option value="{{ $supplier->id }}" {{ $supplier->id == old('supplier_id') ? 'selected' : '' }}>{{ $supplier->tin . ' | ' . $supplier->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                <a href="{{ route('dashboard.supplier.create') }}" class="btn btn-primary w-auto !h-full">
                    ...
                    <div class="col-span-2">
                        <label for="invoice_number" class="label-block">No. Invoice Agen</label>
                        <input type="text" name="invoice_number" id="invoice_number" class="@error('invoice_number') is-invalid @enderror form-control" placeholder="" value="{{ old('invoice_number') ?? '' }}" required>
                    ...
                    <div class="col-span-1">
                        ...
                        <input type="text" name="tax" id="tax" class="@error('tax') is-invalid @enderror form-control numeric !text-left" placeholder="" value="{{ old('tax') ?? 0 }}" maxlength="3" required>
                        @error('tax')
                            <span class="invalid-feedback" category="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                    ...
                    <div class="col-span-1">
                        ...
                        <input type="date" name="date" id="date" class="@error('date') is-invalid @enderror form-control" placeholder="" value="{{ old('date') ?? date('Y-m-d') }}" required>
                        @error('date')
                            <span class="invalid-feedback" category="alert">
                                <strong>{{ $message }}</strong>
                            </span>
...
@push('script')
    ...
    @foreach (old('procurement_items') ?? [] as $procurementItem)
        <script>
