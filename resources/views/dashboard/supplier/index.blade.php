...
@section('content')
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            <li aria-current="page">
                <div class="flex items-center">
                    <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">Agen</span>
                </div>
            </li>
        </ol>
    </nav>
                ...
                <table class="table">
                    <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th scope="col">
                                <a href="{{ request()->fullUrlWithQuery(['orderBy' => 'tin', 'orderType' => request()->orderBy == 'tin' && request()->orderType == 'asc' ? 'desc' : 'asc']) }}" class="inline-flex items-center gap-x-1">
                                    NPWP
                                    <i class="fa {{ request()->orderBy != 'tin' ? 'fa-sort' : (request()->orderBy == 'tin' && request()->orderType == 'asc' ? 'fa-sort-up' : 'fa-sort-down')  }} w-3 h-3 ml-1"></i>
                                </a>
                            </th>
    ...
    <form id="form-delete" action="{{ route('dashboard.supplier.destroy', ['supplier' => 0]) }}" method="POST" class="hidden">
        @csrf
        @method('DELETE')
    </form>
@endsection
